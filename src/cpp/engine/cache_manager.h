...
public:
    ...
    // Writes raw float frame data into a pre-allocated C++ buffer slot.
    // Automatically replaces the slot furthest from the playhead if cache ceiling is reached.
    void write_frame(int frame_index, int width, int height, int channels, const float* pixel_data, size_t data_size);
    
    // Retrieves direct pointer of cached frame. Returns nullptr if miss.
    const float* get_frame_data(int frame_index, int& width, int& height, int& channels);
